 ...
 * Verifica che la logica di pulizia elimini i dati scaduti e conservi quelli
 * recenti.
 *
...
class DataCleanupServiceIntegrationTest {
	...
	void testCleanupOldCommands_deletesOldCompletedCommands() {
		...
		// I comandi DELIVERED/FAILED più vecchi devono essere eliminati (retention 7
		// gg)
		assertTrue(
				commandRepo.findByDeviceIdAndStatusOrderByCreatedAtAsc("cleanup-cmd-dev",
						CommandEntity.CommandStatus.DELIVERED).isEmpty(),
				"I comandi DELIVERED vecchi devono essere eliminati");
		assertTrue(
				commandRepo.findByDeviceIdAndStatusOrderByCreatedAtAsc("cleanup-cmd-dev",
						CommandEntity.CommandStatus.FAILED).isEmpty(),
				"I comandi FAILED vecchi devono essere eliminati");
		// I comandi PENDING recenti devono essere conservati
		assertNotNull(commandRepo.findByDeviceIdAndStatusOrderByCreatedAtAsc("cleanup-cmd-dev",
				CommandEntity.CommandStatus.PENDING));
		assertTrue(
				!commandRepo.findByDeviceIdAndStatusOrderByCreatedAtAsc("cleanup-cmd-dev",
						CommandEntity.CommandStatus.PENDING).isEmpty(),
				"I comandi PENDING recenti devono essere conservati");
	}
	...
	void testCleanupOldCommands_recentCommands_notDeleted() {
		...
		assertTrue(
				!commandRepo.findByDeviceIdAndStatusOrderByCreatedAtAsc("cleanup-recent-cmd",
						CommandEntity.CommandStatus.DELIVERED).isEmpty(),
	...
	void testCleanupOldProcessingMetrics_deletesExpiredMetrics() {
		...
		assertTrue(metricsRepo.findAll().stream().noneMatch(m -> "cleanup-metrics-dev".equals(m.getDeviceId())),
				"Le metriche più vecchie di 90 giorni devono essere eliminate");
		// Metrics recenti (30 gg) devono essere conservate
		assertTrue(metricsRepo.findAll().stream().anyMatch(m -> "cleanup-metrics-recent".equals(m.getDeviceId())),
	...
	void testManualCleanup_returnsSuccessReport() {
		...
		assertTrue(!report.getEndTime().isBefore(report.getStartTime()), "L'end time deve essere >= start time");
	}
	...
	private CommandEntity buildCommand(String deviceId, CommandEntity.CommandStatus status, LocalDateTime createdAt) {
