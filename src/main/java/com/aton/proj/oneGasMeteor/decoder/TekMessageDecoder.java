...
//		decode.getDiagnosticInfo().setActivation(contactReason.getActivation());
	...
	private void decodeDiagnosticInfo(byte[] payload, DecodedMessage decode) {
		...
		decode.getDiagnosticInfo().setLteActive((payload[6] & 0x40) != 0);
	}
	...
	private void decodeDiagnosticData(byte[] payload, DecodedMessage decode) {
		...
		case 8, 23, 24, 27, 28 -> { // TEK822V1, TEK822V1BTN, TEK822V2, TEK898V2, TEK898V1
			...
			if (fwVersion > 3.0) {
				...
				diagnosticInfo.setLastErrorCode(((payload[20] & 0xFF) << 8) | (payload[21] & 0xFF));
			}
		}
		...
		default -> diagnosticInfo.setEnergyUsedLastContactMaSeconds(((payload[20] & 0xFF) << 8) | (payload[21] & 0xFF));
		}
	...
	private void decodeMeasurementData(TelemetryMessage msg, byte[] payload, DecodedMessage decode, int msgType) {
		...
		// Estrai RTC dal payload (PDF sezione 2.2.2)
		// Ore: minor 5 bits di byte 19 (es: 0x97 -> 10111 = 23)
		...
		for (int i = 0; i < 28; i++) {
			...
			int filter = (payload[j] & 0xFF) + (payload[j + 1] & 0xFF) + (payload[j + 2] & 0xFF)
					+ (payload[j + 3] & 0xFF);
			...
			double temperatureC = ((payload[j + 1] & 0x7F) / 2.0) - 30.0;
			...
			long timestamp = baseTimestampMs - (loggerSpeedMs * i);
